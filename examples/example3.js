 ...
 * + and - Keys  for Zoom (Yoosee-style burst: move then immediate stop)
...
new Cam({
	...
	cam_obj.imagingGetMoveOptions({}, function (err, data, xml) {
		if (err || !data) {
			console.log('Focus keys (n/f) unavailable: imaging service not supported on this camera.');
			return;
		}
	...
	function read_and_process_keyboard () {
		...
		console.log('');
		console.log('Use cursor keys for pan/tilt, +/- for zoom. q to quit');
		...
		process.stdin.on('keypress', function (ch, key) {
			...
			if (key && key.name == 'up') {
				...
				moveZoom(ch == '-' ? -0.5 : 0.5, ch == '-' ? 'zoom out' : 'zoom in');
	...
	function move (x_speed, y_speed, zoom_speed, msg) {
		...
		// Step 2 - Send the Pan/Tilt/Zoom 'move' command.
		// Step 3 - In the callback, re-enable the keyboard
		...
		const isTilt = y_speed !== 0 && x_speed === 0 && zoom_speed === 0;
		const moveOptions = {
			x: x_speed,
			y: y_speed,
			zoom: zoom_speed,
			timeout: MOVE_DURATION_MS,
			onlySendPanTilt: isTilt,
			omitZeroPanTilt: isTilt
		};

		function onMoveDone (err) {
			if (err) {
				console.log(err);
			} else {
		...
		function sendMoveWithRetry () {
			cam_obj.continuousMove(moveOptions, function (err) {
				if (err) {
					onMoveDone(err);
					return;
				}
				setTimeout(function () {
					cam_obj.continuousMove(moveOptions, onMoveDone);
				}, MOVE_RETRY_MS);
			});
		}

		// This camera ignores the first tilt command; send the same move twice per keypress.
		if (isTilt) {
			sendMoveWithRetry();
		} else {
			cam_obj.continuousMove(moveOptions, onMoveDone);
	...
	/**
	 * Yoosee cameras accept ContinuousMove but often ignore it unless the HTTP
	 * request is not awaited; stop must follow immediately (see community PTZ
	 * controllers). Pan/tilt work with the normal callback path; zoom uses this.
	 */
	function yooseeFireAndForget (action, body) {
		return new Promise(function (resolve) {
			const ptzUri = cam_obj.uri && cam_obj.uri.ptz;
			if (!ptzUri) {
				resolve(new Error('PTZ service URI not available'));
				return;
			}
			const req = http.request({
				hostname: cam_obj.hostname,
				port: cam_obj.port,
	...
	function moveZoom (zoom_speed, msg) {
		ignore_keypress = true;
		if (stop_timer) { clearTimeout(stop_timer); }

		console.log('sending zoom command ' + msg);
		lastCommand = 'PTZ';

		const token = cam_obj.activeSource.profileToken;
		const moveBody = cam_obj._envelopeHeader() +
			'<ContinuousMove xmlns="http://www.onvif.org/ver20/ptz/wsdl">' +
				'<ProfileToken>' + token + '</ProfileToken>' +
				'<Velocity><Zoom x="' + zoom_speed + '" xmlns="http://www.onvif.org/ver10/schema"/></Velocity>' +
			'</ContinuousMove>' +
			cam_obj._envelopeFooter();
		const stopBody = cam_obj._envelopeHeader() +
		...
		function burstOnce (done) {
			yooseeFireAndForget('ContinuousMove', moveBody)
				.then(function () { return yooseeFireAndForget('Stop', stopBody); })
				.then(done);
		}

		function onZoomDone () {
			console.log('zoom command sent ' + msg);
			ignore_keypress = false;
		}

		// Same double-send workaround as tilt on this firmware.
		burstOnce(function () {
			setTimeout(function () {
				burstOnce(onZoomDone);
			}, MOVE_RETRY_MS);
		});
	}
	...
	function focus (msg) {
			...
			cam_obj.setImagingSettings(options,
				...
				function (err, stream, xml) {
					if (err) {
						...
						stop_timer = setTimeout(stop, MOVE_DURATION_MS);
					}
					resumeKeyboard();
				});
			return;
		...
		if (msg == "near" || msg == "far") {
			if (!hasContinuousFocus) {
				console.log('Continuous focus not supported on this camera (imaging service unavailable).');
				resumeKeyboard();
				return;
			}
			...
			lastCommand = "FOCUS";
			let speed = 0;
			if (msg == "near") speed = focusMinSpeed * 0.25;
			if (msg == "far") speed = focusMaxSpeed * 0.25;
			let continuous = {
				speed: speed
			};
			let options = { continuous: continuous };
			let keyboardResumed = false;
			...
			// Imaging requests on this camera often hang; always recover keyboard.
			const focusTimeout = setTimeout(function () {
				console.log('focus command timed out ' + msg);
				resumeOnce();
			}, cam_obj.timeout || 10000);
		...
		console.log('Unknown focus command ' + msg);
		resumeKeyboard();
	}
