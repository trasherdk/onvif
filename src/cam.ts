import extendDevice from './device.js';
import extendEvents from './events.js';
import extendMedia from './media.js';
import extendPtz from './ptz.js';
import extendImaging from './imaging.js';
import extendRecording from './recording.js';
import extendReplay from './replay.js';
 ...
 * @private
 */
function urlRequestPath(parsedUrl) {
	return parsedUrl.pathname + parsedUrl.search;
}

/**
 * Some cameras return scrambled capability keys but correct path names in XAddr.
 * @param {Object<string, URL>} uri
 * @returns {Object<string, URL>}
 * @private
 */
function normalizeServiceUris(uri) {
	if (!uri) {
		return uri;
	}
	const servicePathMap = [
		['ptz_service', 'ptz'],
		['media2_service', 'media2'],
		['media_service', 'media'],
	...
	Object.keys(uri).forEach(function(key) {
		const parsed = uri[key];
		if (!parsed || !parsed.pathname) {
			normalized[key] = parsed;
			return;
		}
		let mappedKey = key;
		for (let i = 0; i < servicePathMap.length; i++) {
			if (parsed.pathname.includes(servicePathMap[i][0])) {
				mappedKey = servicePathMap[i][1];
				break;
			}
		}
		normalized[mappedKey] = parsed;
	});
	return normalized;
}

/**
 * Fill standard ONVIF service paths when GetServices/GetCapabilities omit or scramble them.
 * @param {Object<string, URL>} uri
 ...
 * @private
 */
function fillMissingServiceUris(uri, hostname, port) {
	if (!uri || !hostname) {
		return uri;
	}
	...
	Object.keys(defaults).forEach(function(key) {
		if (!uri[key]) {
			uri[key] = new URL(base + defaults[key]);
		}
	});
	return uri;
}
 ...
 * });
 */
let Cam = function(options, callback) {
...
Cam.prototype._requestPart2 = function(options, callback) {
	...
	let reqOptions = options.url || {
		...
		path: options.service ?
			(this.uri && this.uri[options.service] ? urlRequestPath(this.uri[options.service]) : options.service) : this.path,
	...
	const req = httpLib.request(reqOptions, function(res) {
		...
		res.on('end', function() {
			...
			callbackExecuted = true;
			let xml = Buffer.concat(bufs, length).toString('utf8');
			/**
	...
	req.on('error', function(err: NodeJS.ErrnoException) {
		...
		if (err.code === 'ECONNREFUSED' && String(err.errno) === 'ECONNREFUSED' && err.syscall === 'connect') {
			callback(err);
			/* network error */
		} else if (err.code === 'ECONNRESET' && String(err.errno) === 'ECONNRESET' && err.syscall === 'read') {
			callback(err);
		} else {
			callback(err);
		}
...
function isConnectionError(err) {
	return err && (err.code === 'ECONNRESET' || err.code === 'EPIPE' || err.message === 'socket hang up');
}
 ...
 * @private
 */
function applySystemDateAndTimeResponse(cam, data, xml, callback) {
	try {
		...
		if (!cam.timeShift) {
			cam.timeShift = time.getTime() - (process.uptime() * 1000);
		}
		callback.call(cam, null, time, xml);
	} catch (err) {
		callback.call(cam, err, null, xml);
	}
}

/**
 * Some cameras drop the connection instead of answering GetSystemDateAndTime.
 * Assume local clock matches the device so WS-Security timestamps still work.
 * @param {Cam} cam
 * @param {Cam~DateTimeCallback} callback
 * @private
 */
function applyLocalTimeShift(cam, callback) {
	const time = new Date();
	if (!cam.timeShift) {
		cam.timeShift = time.getTime() - (process.uptime() * 1000);
	}
	callback.call(cam, null, time);
}
...
Cam.prototype.getSystemDateAndTime = function(callback) {
	...
	const requestAuthenticated = function(finishOnConnectionError) {
		this._request({
			...
			if (!err) {
				return applySystemDateAndTimeResponse(this, data, xml, callback);
			}
			if (finishOnConnectionError && isConnectionError(err)) {
				return applyLocalTimeShift(this, callback);
			}
	...
	this._request({
		...
		const needsAuth = (statusCode && statusCode === 401)
			|| (xml && (xmlLowerCase.includes('sender not authorized') || xmlLowerCase.includes('unauthorized') || xmlLowerCase.includes('notauthorized')));
		if (needsAuth && this.username && this.password) {
			return requestAuthenticated(false);
		}
		if (isConnectionError(err)) {
			if (this.username && this.password) {
				return requestAuthenticated(true);
			}
			return applyLocalTimeShift(this, callback);
		}
		callback.call(this, err, null, xml);
	...
	this._request({
		...
		if (!err) {
			...
			if (this.capabilities.extension) {
				Object.keys(this.capabilities.extension).forEach(function(ext) {
					...
					if (this.capabilities.extension[ext].XAddr) {
						this.uri[ext] = this._parseUrl(this.capabilities.extension[ext].XAddr);
					}
				}.bind(this));
			}
			...
			if ((this.uri['replay']) && !this.uri['recording']) {
				let tempRecorderXaddr = this.uri['replay'].href.replace('replay', 'recording');
				console.warn("WARNING: Adding " + tempRecorderXaddr + " for bad Profile G device");
				this.uri['recording'] = this._parseUrl(tempRecorderXaddr);
			}
			this.uri = normalizeServiceUris(this.uri);
			fillMissingServiceUris(this.uri, this.hostname, this.port);
		}
		if (callback) {
...
Cam.prototype.getActiveSources = function() {
	...
	this.videoSources.forEach(function(videoSource, idx) {
		...
		let videoSrcToken = videoSource.$.token,
		...
		if (this.defaultProfiles[idx].videoEncoderConfiguration) {
			let configuration = this.defaultProfiles[idx].videoEncoderConfiguration;
			...
			this.services.forEach((service) => {
				...
				if (Object.prototype.hasOwnProperty.call(service, 'namespace') && Object.prototype.hasOwnProperty.call(service, 'XAddr')) {
					...
					if (parsedNamespace.hostname === 'www.onvif.org') {
						let namespaceSplitted = parsedNamespace.pathname.substring(1).split('/'); // remove leading Slash, then split
...
Cam.prototype._parseUrl = function(address) {
	...
	// we substitute hostname and port from settings
	if (this.preserveAddress && (this.hostname !== parsedAddress.hostname || String(this.port) !== parsedAddress.port)) {
		parsedAddress.hostname = this.hostname;
		parsedAddress.port = String(this.port);
	}
