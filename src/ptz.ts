import { linerase } from './utils.js';
import type { CamConstructor } from './cam.js';

export default function extendPtz(Cam: CamConstructor): void {
		...
		this._request({
			...
			if (!err) {
				...
				data[0]['getConfigurationsResponse'].forEach(function(configuration) {
					//let pTZConfiguration = configuration['PTZConfiguration'][0]; // This was a bug. only used first PTZConfiguration
	...
	Cam.prototype.getConfigurationOptions = function(configurationToken, callback) {
		this._request({
			...
			let configOptions;
			if (!err) {
				let sp = data[0]['getConfigurationOptionsResponse'][0]['PTZConfigurationOptions'][0]['spaces'][0];
	 ...
	 * @param {boolean} [options.omitZeroPanTilt] Omit pan/tilt axes that are zero from the ONVIF XML
	...
	Cam.prototype.continuousMove = function(options, callback) {
		...
		if (options.onlySendPanTilt) {delete options.zoom;}
		if (options.onlySendZoom) { delete options.x; delete options.y; }
		if (options.omitZeroPanTilt) {
			if (options.x === 0) {delete options.x;}
			if (options.y === 0) {delete options.y;}
		}
	...
	Cam.prototype._panTiltZoomVectors = function(ptz) {
		...
		return ptz
			...
			(ptz.x !== undefined || ptz.y !== undefined
				? '<PanTilt' +
				(ptz.x !== undefined ? ' x="' + ptz.x + '"' : '') +
				(ptz.y !== undefined ? ' y="' + ptz.y + '"' : '') +
				xyspace + ' xmlns="http://www.onvif.org/ver10/schema"/>'
				: '') +
			(ptz.zoom !== undefined
