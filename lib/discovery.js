...
function ipsInSubnet(ip, netmask) {
	var network = ipToLong(ip) & ipToLong(netmask);
	var broadcast = network | (~ipToLong(netmask) >>> 0);
	var hosts = [];
...
function probeMatchList(data) {
	...
	var matches = data.probeMatches.probeMatch;
			...
			parseSOAPString(msg.toString(), function(err, data, xml, _statusCode) {
				...
				if (err || !data[0] || !data[0].probeMatches) {
					...
					probeMatchList(data).forEach(function(match) {
						var camAddr = match.endpointReference && match.endpointReference.address;
						var xaddrsRaw = probeMatchXAddrs(match);
						...
						if (!cams[camAddr]) {
							...
							if (options.resolve !== false) {
								...
								var camUris = xaddrsRaw.split(' ').filter(Boolean).map(function(url) { return new URL(url); }),
	...
	const bindingCallback = function(err) {
		if (err) {
			...
			if (bindAddress) {
				...
				try {
					socket.addMembership('239.255.255.250', bindAddress);
				} catch (_memberErr) {
					// ignore
				}
			}
			...
			var targets = [{ host: '239.255.255.250', port: 3702 }];
			if (bindAddress && bindNetmask && options.subnetBroadcast !== false) {
				targets.push({ host: subnetBroadcast(bindAddress, bindNetmask), port: 3702 });
			}
			...
			if (options.subnetScan && bindAddress && bindNetmask) {
				ipsInSubnet(bindAddress, bindNetmask).forEach(function(host) {
					targets.push({ host: host, port: 3702 });
				});
			}

			targets.forEach(function(target) {
				socket.send(request, 0, request.length, target.port, target.host, function(sendErr) {
